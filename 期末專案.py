...
undertaker = pygame.image.load(os.path.join("undertaker.png")).convert()#載入圖片路徑
bullet_image = pygame.image.load(os.path.join("bullet.jpg")).convert()#載入圖片路徑
...
class Plane(pygame.sprite.Sprite):#用plane繼承sprite類別
    def __init__(self):
        ...
        self.image = pygame.transform.scale(undertaker,(90,165))#死神機甲圖片，改規格大小
    ...
    def update(self):
        key_pressed = pygame.key.get_pressed()#pygame.key.get_pressed()會回傳布林值，判斷鍵盤上有沒有按鍵被按下去
...
class Army(pygame.sprite.Sprite):#用rock繼承sprite類別
    def __init__(self):
        ...
        self.rect = self.image.get_rect()#用來定位圖片有x,y(框起來)
        self.radius = 25
        self.rect.x = random.randrange(0, WIDTH - self.rect.width) #隨機在不同x座標生成
...
while run:
    ...
    for event in pygame.event.get():#取得輸入
        if event.type == pygame.QUIT:#當畫面又上的框框被按下
            run = False
        elif event.type == pygame.KEYDOWN:#當鍵盤按鍵被按下的時候
            if event.key == pygame.K_UP:
                plane.shoot()
    
    all_sprite.update()
    hits = pygame.sprite.groupcollide(armys,bullets,True,True)#判斷敵人跟子彈有沒有撞到一起，第三個是判斷敵人碰撞後要不要刪掉
    ...
    hits = pygame.sprite.spritecollide(plane,armys,True,pygame.sprite.collide_circle)#判斷死神跟子彈有沒有撞到一起
